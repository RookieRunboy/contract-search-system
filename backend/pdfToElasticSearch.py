...
class JSONToElasticsearch:
    def __init__(self, es_host: str ="http://localhost:9200", model_name: str ="bge-m3", index_name: str ="contracts_unified"):
        ...
        try:
            self.es = Elasticsearch([es_host], headers={"Accept": "application/vnd.elasticsearch+json; compatible-with=8"})
            try:
                if not self.es.ping():
                    print("警告: Elasticsearch连接失败，但服务将继续启动")
            except Exception as exc:
                print(f"警告: 无法检测Elasticsearch连接状态: {exc}")
        except Exception as e:
            print(f"Elasticsearch 初始化错误: {str(e)}")

        try:
            self.embedding_client = RemoteEmbeddingClient(model=model_name)
        except Exception as e:
            # 缺少向量服务时保留None，后续索引时自动降级为仅文本模式
            print(f"向量服务未初始化，将跳过向量化: {e}")
        ...
        try:
            ...
            if self.embedding_client:
                try:
                    ...
                    print(f"警告: 生成向量失败，改为仅索引文本: {vector_exc}")
            else:
                print("提示: 向量服务未配置，跳过向量化，仅保存文本内容")
    ...
    def json_to_elasticsearch(
        ...
        vectorization_skipped = self.embedding_client is None
        vector_message = "向量服务未配置，已跳过向量化，仅保存文本内容" if vectorization_skipped else None
        ...
        try:
            ...
            result = {
                ...
                "vectorization_skipped": vectorization_skipped,
            }
