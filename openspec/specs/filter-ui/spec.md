...
### Requirement: Sync Filters from Smart Search
The system SHALL update the Filter Bar UI to reflect filters extracted by Smart Search or other external sources.

#### Scenario: Populate Filters from AI
- **WHEN** 用户执行"智能搜索"且 AI 成功提取了筛选条件（如"2023年合同"）
- **THEN** 搜索执行
- **AND** 筛选器列表会自动增加一行"签订日期"
- **AND** 该行的值被自动设定为 AI 解析出的时间范围
- **AND** 原有的手动筛选条件视逻辑保留或合并

---

### Requirement: Visual Distinction for AI Filters
The system SHALL visually distinguish filters that were automatically generated by AI versus those manually added by the user.

#### Scenario: AI Filter Appearance
- **GIVEN** 一个由智能搜索生成的筛选器行
- **THEN** 该行的最外圈应有特殊的视觉样式（如紫色光晕或边框）
- **AND** 与普通用户手动添加的行形成明显区分，体现"AI辅助"的心智模型

#### Scenario: User Interaction with AI Filter
- **WHEN** 用户手动修改了 AI 生成的筛选器行的值
- **THEN** 该行不再视为 AI 生成（或者样式保持但逻辑允许修改，状态变为用户确认）
- **OR** 简单起见，样式保持展示"来源是AI"，但允许用户自由修改
